# modules/systemLevel/accounts/default.nix
{
  ...
  hasAnyIncludedRole = u: let
    rs = userInfos.${u}.roles or [];
  in
    (includeRoles == ["*"]) || lib.any (r: lib.elem r includeRoles) rs;
  ...
  in
    (u == mainUser) || adminFlag || lib.elem wheelRole rs;
  ...
  roleGroups = u: let
    rs = userInfos.${u}.roles or [];
  in
    lib.unique (lib.concatMap (r: (extraGroupsByRole.${r} or [])) rs);
...
in {
  ...
  home-manager.extraSpecialArgs = {inherit inputs systemInfo userInfos repoRoot;};
